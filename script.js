...
function insertHTML(event) {
  ...
  element_content.value =
    VALUE.slice(0, START) + target_tag.complete + VALUE.slice(END);
  element_content.selectionStart = element_content.selectionEnd =
    START + target_tag.cursor_offset;
...
function getCLASS() {
  return (
    'class="' +
    prompt('Please enter the classes (seperated with a space):') +
    '"'
  );
}
